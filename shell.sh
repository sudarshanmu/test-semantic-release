...
if [ "$release_type" = "minor" ]; then
  ...
  new_build_number=$(echo $current_build_number | awk -F '.' '{print ($1 + 1)"."0"."0"."0}')
  ...
  new_marketing_version=$(echo $current_marketing_version | awk -F '.' '{print ($1 + 1)"."0"."0}')
  ...
  latest_version=$(echo $old_version | awk -F '.' '{print ($1 + 1)"."0"."0}')
